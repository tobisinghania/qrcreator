...
class MaskUtil {
    ...
    static int applyMaskPenaltyRule4(ByteMatrix matrix) {
        int numDarkCells = 0
        byte[][] array = matrix.array
        matrix.height.times { int y ->
            byte[] arrayY = array[y]
            matrix.width.times { int x ->
                if (arrayY[x] == 1) {
                    numDarkCells++
                }
            }
        ...
        int fivePercentVariances = (int)(Math.abs(numDarkCells * 2 - numTotalCells) * 10 / numTotalCells)
    ...
    static boolean getDataMaskBit(int maskPattern, int x, int y) {
        ...
        int temp
        switch (maskPattern) {
            case 0: intermediate = (y + x) & 0x1; break
            case 1: intermediate = y & 0x1; break
            case 2: intermediate = x % 3; break
            case 3: intermediate = (y + x) % 3; break
            case 4: intermediate = (((y / 2) as int) + ((x / 3) as int)) & 0x1; break
            case 5:
                temp = y * x
